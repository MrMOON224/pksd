/**
 * Purchase Order List Logic - Excel Grid Version
 * Derived from purchase-list.js but only pulls orders (status='Ordered')
 */

const SCHEMA = [
    { id: 'date', label: 'Date', width: '8%', align: 'center' },
    { id: 'voucher', label: 'Order Ref', width: '12%', minWidth: '120px' },
        ...
        state.columns.forEach(col => {
            ...
            switch (col.id) {
                case 'date':
                    ...
                    break;
                case 'voucher':
                    container.innerHTML = `<span class="voucher-link" onclick="viewOrderDetail('${p.id}')">${p.reference_no || 'N/A'}</span>`;
                    break;
                case 'supplier':
